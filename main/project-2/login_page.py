...
class login_page:
    def __init__(self, root):
        ...
        self.frame1 = Frame(self.window, bg="#3AAFA9")
        self.frame1.place(x=0, y=0, width=450, relheight = 1)

        label1 = Label(self.frame1, text= "R.", font=("kunstler script", 40, "bold"), bg="#3AAFA9", fg="yellow").place(x=100,y=300)
        label2 = Label(self.frame1, text= "legit", font=("times new roman", 35, "bold"), bg="#3AAFA9", fg="white").place(x=162,y=300)
        label3 = Label(self.frame1, text= "Its's just the beginning", font=("times new roman", 13, "bold"), bg="yellow", fg="brown4").place(x=100,y=360)
    ...
    def login_func(self):
        if self.email_entry.get()=="" or self.password_entry.get()=="":
            ...
            try:
                connection=pymysql.connect(host="localhost", user="root", password="Tn55bu9445@2020", database="student_database")
                ...
                if row == None:
                    ...
                    messagebox.showinfo("Success","Welcome to the My Website",parent=self.window)
